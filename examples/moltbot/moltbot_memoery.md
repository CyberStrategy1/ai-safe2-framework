# AI SAFE² Memory Protocol for Moltbot/Clawdbot

**Version:** 2.1  
**Purpose:** Persistent security controls embedded in agent memory  
**Deployment:** Drop this file in `~/.clawdbot/memories/` and restart
...
3. Run security scan: `clawdbot security audit --deep`
...
**If you suspect your Moltbot has been compromised:**

**1. STOP - Activate Safe Mode Immediately**
```bash
# Kill all Moltbot processes
pkill -9 -f clawdbot
...
sudo gcore -o /tmp/moltbot-dump $(pidof clawdbot)

# Archive logs
tar -czf /tmp/moltbot-logs-$(date +%Y%m%d-%H%M%S).tar.gz ~/.clawdbot/logs/
...
```bash
# Review audit logs
cat ~/.clawdbot/logs/audit.log | grep -A5 -B5 "suspicious_pattern"

# Check for unauthorized access
cat ~/.clawdbot/logs/gateway.log | grep "401\|403\|unauthorized"

# Identify data exfiltration
cat ~/.clawdbot/logs/audit.log | grep "outbound\|upload\|POST"
```
...
|---------|------|---------|
| 2.1 | 2026-01-30 | Initial Moltbot-specific release |
