# Moltbot Security Troubleshooting Guide

**Quick fixes for common Moltbot/Clawdbot security issues**
...
- Can access Moltbot UI from external IP address
- Moltbot shows up in Shodan searches
...
- Can see API keys when viewing logs: `cat ~/.clawdbot/logs/gateway.log`
...
# 3. Restart Moltbot
pkill -f clawdbot && clawdbot gateway --bind 127.0.0.1

# 4. Verify
tail ~/.clawdbot/logs/gateway.log
...
# 2. Update Moltbot config with new keys

# 3. Delete old keys from provider

# 4. Securely delete old logs
shred -u ~/.clawdbot/logs/*.log
```
...
**Symptoms:**
- Scanner reports: "clawdbot.json has permissions 644"
...
- Moltbot can execute shell commands
...
- `~/.clawdbot/logs/` directory empty or doesn't exist
...
cd ~/.clawdbot/memories/
curl -O https://raw.githubusercontent.com/CyberStrategyInstitute/ai-safe2-framework/main/examples/moltbot/moltbot_memory.md
...
**Symptoms:**
- Scanner reports: "Anthropic API key hardcoded in clawdbot.json"
...
```bash
# macOS
security add-generic-password -a moltbot -s anthropic_key -w "sk-ant-..."

# Linux (gnome-keyring)
secret-tool store --label='Anthropic API Key' service moltbot account anthropic
```
...
### Issue: Scanner Can't Find Moltbot

**Symptoms:**
```
ERROR: Path not found: /home/user/.clawdbot
```
...
python3 scanner.py --path /actual/path/to/.clawdbot
```
...
### Moltbot Won't Start After Changes

**Symptoms:** Made config changes, now Moltbot won't start
...
clawdbot gateway --bind 127.0.0.1 --verbose
```

---

### Can't Access Moltbot UI After Hardening

**Symptom:** Secured Moltbot, now can't access web UI
...
**Symptoms:**
- Added `moltbot_memory.md` to `/memories/` but bot still accepts dangerous commands

**Troubleshooting:**

1. **Verify file is in correct location**
```bash
ls -la ~/.clawdbot/memories/moltbot_memory.md
# File should exist
```

2. **Check file permissions**
```bash
chmod 644 ~/.clawdbot/memories/moltbot_memory.md
```

3. **Restart Moltbot completely**
```bash
pkill -9 -f clawdbot
clawdbot gateway --bind 127.0.0.1
```
...
tail -100 ~/.clawdbot/logs/audit.log
```

2. **Run scanner with verbose output:**
```bash
python3 scanner.py --path ~/.clawdbot --output detailed-report.txt
```
...
**If you suspect your Moltbot has been hacked:**
...
```bash
# Copy logs
tar -czf /tmp/moltbot-incident-$(date +%Y%m%d-%H%M%S).tar.gz ~/.clawdbot/logs/

# Copy configs
tar -czf /tmp/moltbot-config-$(date +%Y%m%d-%H%M%S).tar.gz ~/.clawdbot/
```
...
```bash
# Review audit logs
cat ~/.clawdbot/logs/audit.log | grep -E "exec|browser|DELETE|DROP"

# Check for unauthorized access
cat ~/.clawdbot/logs/gateway.log | grep -E "401|403|unauthorized"

# Look for data exfiltration
cat ~/.clawdbot/logs/audit.log | grep -E "upload|POST|curl"
```
...
rm -rf ~/.clawdbot/
# Reinstall Moltbot from official source
